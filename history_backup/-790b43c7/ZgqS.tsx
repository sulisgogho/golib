...
export default async function LaporanPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  // 1. CEK SESI LOGIN
  ...
  // 2. BACA FILTER WAKTU DARI URL
  const resolvedParams = await searchParams
  const currentRange = resolvedParams.range || 'bulan_ini'

  // 3. TENTUKAN BATAS TANGGAL BERDASARKAN FILTER
  const now = new Date()
  let startDate = new Date(now.getFullYear(), now.getMonth(), 1) // Default: Awal bulan ini

  if (currentRange === '7_hari') {
    startDate = new Date(now)
    startDate.setDate(now.getDate() - 7) // Tarik mundur 7 hari
  }

  // Format teks bulan dinamis (contoh: "APR 2026")
  const monthYearText = new Intl.DateTimeFormat('id-ID', { month: 'short', year: 'numeric' }).format(now).toUpperCase()
  ...
  // Data pengguna dinamis
  const firstName = account?.client_name.toUpperCase().split(' ')[0] || 'TYO'
  const initial = firstName.charAt(0)
  const accountName = account?.client_name || 'CatetDuit' // NAMA AKUN ASLI
    ...
    <div className="w-full h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      <header className="flex justify-between items-center px-6 py-5 bg-[#F7F9F8]">
        <div className="flex items-center gap-2">
          ...
          {/* NAMA AKUN DINAMIS (Bukan "The Atelier" lagi) */}
      ...
      <div className="px-6 py-2 flex justify-between items-center">
        <div className="flex bg-gray-100/80 p-1 rounded-full">
          <Link href="/laporan?range=bulan_ini" className={`text-[10px] font-bold px-4 py-1.5 rounded-full transition-colors ${currentRange === 'bulan_ini' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'}`}>
            Bulan Ini
          </Link>
          <Link href="/laporan?range=7_hari" className={`text-[10px] font-bold px-4 py-1.5 rounded-full transition-colors ${currentRange === '7_hari' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'}`}>
            7 Hari Terakhir
          </Link>
          <button className="text-[10px] font-bold text-gray-500 px-4 py-1.5 rounded-full cursor-not-allowed">Kustom</button>
        </div>
        ...
        <button className="flex items-center gap-1 text-[9px] font-bold text-gray-500 bg-white border border-gray-200 px-3 py-1.5 rounded-full shadow-sm">
          <Calendar className="w-3 h-3" /> {monthYearText}
        </button>
      </div>
        ...
        <div className="bg-white p-6 rounded-[30px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-gray-50">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-[10px] text-gray-500 mt-1">Analisis distribusi pengeluaran {currentRange === '7_hari' ? 'minggu' : 'siklus'} ini</p>
            </div>
