...
import { Settings, LayoutGrid, Plus, BarChart2, Calendar, Sparkles, Download, ArrowRight } from 'lucide-react'
...
export default async function LaporanPage({ searchParams }: { searchParams: Promise<{ range?: string; start?: string; end?: string }> }) {
  ...
  const now = new Date()
  let startDate = new Date(now.getFullYear(), now.getMonth(), 1) // Default Awal Bulan
  let endDate = new Date() // Default Hari Ini
  let dateLabel = '' // Teks untuk ikon kalender di kanan atas
  ...
  if (currentRange === '7_hari') {
    ...
    endDate.setHours(23, 59, 59, 999) // Pastikan mengambil sampai akhir hari tersebut

    // Format teks "10 Apr - 15 Apr"
    ...
    // Format default bulan ini "APR 2026"
  ...
  const transactions = await prisma.transaction.findMany({
    where: {
      ...
      transaction_date: {
        gte: startDate,
        lte: endDate, // Ambil data di antara startDate dan endDate
      },
    },
  ...
  return (
    <div className="w-full h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      ...
      {/* FORM INPUT TANGGAL KUSTOM (Hanya Muncul Jika Tab Kustom Aktif) */}
      {currentRange === 'kustom' && (
        ...
        <div className="bg-white p-6 rounded-[30px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-gray-50">
          ...
          <div className="bg-[#F0FDF4] border border-[#DCFCE7] p-4 rounded-2xl">
            ...
            <p className="text-xs text-[#0B4D3C] leading-relaxed">
              Anda <span className="font-extrabold">12% lebih hemat</span> dibanding bulan lalu. Bersama-sama, Anda telah menabung Rp400k untuk {accountName}.
            </p>
          </div>
        </div>
