...
export default async function AdminDashboardPage() {
 ...
 // 1. Tarik Data Akun untuk Perhitungan Pendapatan & Statistik
  const allAccounts = await prisma.account.findMany({
    select: {
      ...
      account_type: true,
      status: true,
      createdAt: true, // GANTI DARI created_at ke createdAt
    },
    orderBy: { 
      createdAt: 'desc' // GANTI JUGA DI SINI
    }
  })

  // 2. Di bagian filter "User Baru Hari Ini", ganti juga variabelnya:
  const newUsersToday = allAccounts.filter(a => {
    const today = new Date().toDateString()
    // Pastikan pakai a.createdAt sesuai select di atas
    return a.createdAt ? new Date(a.createdAt).toDateString() === today : false
  }).length

  // Ambil 5 akun terbaru untuk ditampilkan di tabel
