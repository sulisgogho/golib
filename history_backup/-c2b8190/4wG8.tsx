...
// Tambahkan baris ini untuk memaksa Next.js selalu merender ulang halaman saat URL berubah
export const dynamic = 'force-dynamic'

// 1. Ubah tipe data searchParams menjadi Promise
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
  // 2. TUNGGU (await) searchParams sebelum dibaca
  ...
  // SIAPKAN MESIN PENCARI (Berdasarkan siapa yang bayar)
  ...
  // 4. TARIK DATA AKUN DAN TRANSAKSI
  ...
  // Total pengeluaran sekarang otomatis menyesuaikan data yang difilter!
  ...
  return (
    <div className="w-full h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      <main className="flex-1 overflow-y-auto scrollbar-hide pb-6">
        ...
        {/* Filter Bulan (Visual Statis Sementara) */}
          ...
          <div className="mb-6">
            {transactions.length === 0 ? (
              <div className="text-center bg-white p-8 rounded-2xl border border-gray-100 mt-4">
                ...
                <p className="text-xs text-gray-400 mt-1">Belum ada pengeluaran untuk kategori ini.</p>
              </div>
            ) : (
