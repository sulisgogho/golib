...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ filter?: string; range?: string; start?: string; end?: string }> }) {
  ...
  // Buat variabel nama partner dalam huruf kecil untuk kebutuhan URL/Filter
  const p1Lower = account.partner_1_name ? account.partner_1_name.toLowerCase() : ''
  const p2Lower = account.partner_2_name ? account.partner_2_name.toLowerCase() : ''

  // --- 3. SUSUN FILTER BERDASARKAN HASPARTNER ---
  const baseWhere: any = { account_id: accountId }

  if (hasPartner) {
    if (currentFilter === p1Lower) {
      baseWhere.paid_by = {
        equals: account.partner_1_name,
        mode: 'insensitive',
      }
    }
    if (currentFilter === p2Lower) {
      baseWhere.paid_by = {
        equals: account.partner_2_name,
        mode: 'insensitive',
      }
    }
  }
    ...
    <div className="w-full h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      <main className="flex-1 overflow-y-auto scrollbar-hide pb-6">
        ...
        <section className="px-6 py-4">
          ...
          {/* FILTER PARTNER DINAMIS */}
          {hasPartner && (
            <div className="flex gap-2 mb-6">
              {['semua', p1Lower, p2Lower].map((f) => {
                if (!f) return null // Guard clause jika nama kosong
                return (
                  <Link
                    key={f}
                    href={`/dashboard?filter=${f}&range=${currentRange}${kustomParams}`}
                    className={`text-xs font-bold px-4 py-2 rounded-full transition-all uppercase ${currentFilter === f ? 'bg-[#0B4D3C] text-white shadow-md' : 'bg-white text-gray-500 border border-gray-200'}`}
                  >
                    {f}
                  </Link>
                )
