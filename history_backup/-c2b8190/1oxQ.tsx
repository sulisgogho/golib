...
import { Bell, Settings, Filter, Utensils, ShoppingBag, LayoutGrid, Plus, BarChart2, ReceiptText } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  // Jika belum login atau tidak ada sesi, paksa kembali ke halaman login
  if (!session) {
    redirect('/login')
  }

  // 2. TARIK DATA DARI DATABASE NEON
  const accountId = (session.user as any).id
  const account = await prisma.account.findUnique({
    where: { id: accountId },
    include: {
      transactions: {
        orderBy: { transaction_date: 'desc' }, // Urutkan dari yang terbaru
        take: 5, // Ambil 5 transaksi terakhir untuk di Dasbor
        include: { category: true }, // Bawa serta data kategorinya
      },
    },
  })
  ...
  // Ambil inisial dan nama depan dari partner 1 (misal: dari "Tyo" jadi "T")
  const firstName = account.partner_1_name?.toUpperCase() || account.client_name.toUpperCase().split(' ')[0]
  const initial = firstName.charAt(0)

  // Hitung total pengeluaran (sementara kita jumlahkan semua transaksi yang ada di database)
  const totalPengeluaran = account.transactions.reduce((sum, trx) => sum + Number(trx.amount), 0)
  ...
  return (
    <div className="h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      <main className="flex-1 overflow-y-auto scrollbar-hide pb-6">
        ...
        {/* FILTER BULAN (Masih Statis untuk UI) */}
          ...
          <div className="mb-6">
            {account.transactions.length === 0 ? (
              ...
              <div className="text-center bg-white p-8 rounded-2xl border border-gray-100 mt-4">
                ...
                <p className="text-xs text-gray-400 mt-1">Mulai catat pengeluaran pertamamu bulan ini.</p>
              </div>
            ) : (
              // Tampilan Jika Ada Transaksi (Looping Data Asli)
              account.transactions.map((trx) => (
                <div key={trx.id} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm mb-3 border border-gray-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-medium mt-0.5">
                        Dibayar oleh <span className="font-bold text-gray-600">{trx.paid_by}</span>
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-sm text-gray-800">Rp{Number(trx.amount).toLocaleString('id-ID')}</p>
                    <p className="text-[9px] font-bold text-[#0B4D3C] tracking-wider uppercase mt-1">{trx.category?.name || 'LAINNYA'}</p>
                  </div>
                </div>
              ))
            )}
