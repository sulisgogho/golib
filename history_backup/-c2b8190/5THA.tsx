...
export default function DashboardPage() {
  return (
    <div className="min-h-screen bg-[#F7F9F8] pb-24 font-sans max-w-md mx-auto relative shadow-xl overflow-hidden">
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white border-t border-gray-100 rounded-t-[30px] px-6 py-4 flex justify-between items-center z-50 pb-8 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <Link href="/dashboard" className="flex flex-col items-center gap-1 text-[#0B4D3C]">
          <LayoutGrid className="w-5 h-5" />
          <span className="text-[9px] font-bold tracking-widest uppercase">BERANDA</span>
        </Link>

        {/* Tombol Tambah Tengah (Floating) */}
        <div className="relative -top-8">
          <Link href="/tambah" className="w-14 h-14 bg-[#0B4D3C] rounded-full flex items-center justify-center text-white shadow-lg border-4 border-[#F7F9F8] hover:scale-105 transition-transform">
        ...
        <Link href="/laporan" className="flex flex-col items-center gap-1 text-gray-400 hover:text-[#0B4D3C] transition-colors">
          ...
          <span className="text-[9px] font-bold tracking-widest uppercase">LAPORAN</span>
        </Link>
      </div>
    </div>
  )
