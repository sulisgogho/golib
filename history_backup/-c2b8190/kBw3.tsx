...
// Tambahkan prop searchParams untuk menangkap URL parameter (contoh: ?filter=el)
export default async function DashboardPage({ searchParams }: { searchParams: { filter?: string } }) {
  ...
  // 2. BACA FILTER YANG SEDANG AKTIF
  const currentFilter = searchParams.filter || 'semua'

  // 3. SIAPKAN MESIN PENCARI (Berdasarkan siapa yang bayar)
