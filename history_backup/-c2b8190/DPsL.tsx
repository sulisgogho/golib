...
import { Bell, Settings, Filter, Utensils, ShoppingBag, LayoutGrid, Plus, BarChart2, ReceiptText, Car, Home, MoreHorizontal, Trash2, Calendar, ArrowRight } from 'lucide-react'
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ filter?: string; range?: string; start?: string; end?: string }> }) {
  ...
  // 2. LOGIKA RENTANG WAKTU (Sama persis dengan Laporan)
  const now = new Date()
  ...
  // String parameter tambahan agar saat pindah filter "Tyo/El", rentang waktunya tidak kereset
  ...
  return (
    <div className="w-full h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      <main className="flex-1 overflow-y-auto scrollbar-hide pb-6">
        ...
        {/* 4. FILTER WAKTU (Gaya Kapsul Laporan) */}
        ...
        <section className="px-6 py-4">
          ...
          {/* 5. FILTER ORANG (Menyimpan Filter Waktu) */}
