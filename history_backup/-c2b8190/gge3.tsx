...
const getCategoryIcon = (categoryName: string) => {
  switch (categoryName) {
    ...
    case "Gaya Hidup": return <ShoppingBag className="w-5 h-5" />;
    case "Transportasi": return <Car className="w-5 h-5" />;
    case "Hunian": return <Home className="w-5 h-5" />;
      ...
      <main className="flex-1 overflow-y-auto scrollbar-hide pb-6">
        ...
        <section className="px-6 py-4">
          <p className="text-[10px] font-bold text-gray-500 tracking-wider uppercase mb-1">
            TOTAL PENGELUARAN {currentFilter === "semua" ? "BULAN INI" : `OLEH ${currentFilter.toUpperCase()}`}
          </p>
          <div className="flex items-baseline gap-1 mb-2">
            <h2 className="text-4xl font-extrabold text-[#0B4D3C] tracking-tight">
        ...
        <section className="px-6 py-4">
          ...
          <div className="flex gap-2 mb-6">
            <Link href="/dashboard?filter=semua" className={`text-xs font-bold px-3 py-1.5 rounded-full transition-colors ${currentFilter === "semua" ? "bg-gray-800 text-white" : "bg-white text-gray-500 border border-gray-200"}`}>Semua</Link>
            <Link href="/dashboard?filter=tyo" className={`text-xs font-bold px-3 py-1.5 rounded-full transition-colors ${currentFilter === "tyo" ? "bg-gray-800 text-white" : "bg-white text-gray-500 border border-gray-200"}`}>Tyo</Link>
            <Link href="/dashboard?filter=el" className={`text-xs font-bold px-3 py-1.5 rounded-full transition-colors ${currentFilter === "el" ? "bg-gray-800 text-white" : "bg-white text-gray-500 border border-gray-200"}`}>El</Link>
          </div>
          ...
          <div className="mb-6">
            {transactions.length === 0 ? (
              ...
              transactions.map((trx) => {
                ...
                // 4. BINDING FUNGSI HAPUS DENGAN ID TRANSAKSI
