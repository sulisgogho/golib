...
export default function TambahPengeluaranPage() {
  ...
  // 1. Pastikan data user ditarik dengan aman
  const user = session?.user as any
  const myName = user?.name?.split(' ')[0] || 'Saya'
  const partnerName = user?.partnerName

  // 2. KUNCI PERBAIKAN: Jangan putuskan 'hasPartner' jika session masih loading
  const hasPartner = status === 'authenticated' && !!partnerName
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-6 pb-10">
          <div className="mb-8">
            ...
            >
              ...
              <div className="relative z-10 bg-white/10 p-3 rounded-full backdrop-blur-sm group-hover:scale-110 transition-transform">{isScanning ? <ScanLine className="w-6 h-6 animate-pulse" /> : <Camera className="w-6 h-6" />}</div>
            </button>
