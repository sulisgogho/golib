...
import { ArrowLeft, Camera, Receipt, Calendar, Wallet, Tag, AlignLeft } from "lucide-react";
  ...
  return (
    <div className="h-screen bg-[#F7F9F8] font-sans max-w-md mx-auto relative shadow-xl flex flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between p-6 bg-white sticky top-0 z-10 border-b border-gray-100">
        ...
        <div className="w-9"></div> {/* Spacer agar judul presisi di tengah */}
      </header>

      {/* KONTEN UTAMA */}
      <main className="flex-1 overflow-y-auto scrollbar-hide p-6 pb-10">
        
        {/* FITUR UNGGULAN: SCAN STRUK OCR */}
        <div className="mb-8">
          <button className="w-full bg-[#0B4D3C] text-white rounded-2xl p-5 shadow-[0_8px_20px_rgba(11,77,60,0.2)] hover:bg-[#083A2D] transition-colors flex items-center justify-between relative overflow-hidden group">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="date"
                  // Default value hari ini (opsional untuk mockup)
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select className="bg-white border border-gray-100 text-gray-800 text-xs font-medium rounded-xl block w-full pl-9 p-4 focus:outline-none focus:ring-2 focus:ring-[#0B4D3C] shadow-sm appearance-none cursor-pointer">
          ...
          <div>
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button 
                onClick={() => setPaidBy("Tyo")}
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${paidBy === "Tyo" ? "bg-white text-[#0B4D3C] shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
              >
                Tyo
              </button>
              <button 
                onClick={() => setPaidBy("El")}
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${paidBy === "El" ? "bg-white text-[#0B4D3C] shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
              >
                El
              </button>
            </div>
      ...
      <div className="bg-white border-t border-gray-100 p-6 z-20">
        <button className="w-full text-white bg-[#0B4D3C] hover:bg-[#083A2D] font-bold rounded-xl text-sm px-5 py-4 transition-colors flex justify-center items-center shadow-[0_8px_16px_rgba(11,77,60,0.15)]">
          Simpan Transaksi
        </button>
      </div>
