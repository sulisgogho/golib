...
export default function TambahPengeluaranPage() {
  ...
  const partnerName = user?.partnerName; // Sudah otomatis berisi nama pasangan (bukan nama sendiri)
  ...
  // Effect untuk set default paidBy setelah session load
  useEffect(() => {
    if (user?.name) {
      setPaidBy(user.name.split(' ')[0])
    }
  }, [user])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Gunakan paidBy dari state agar sinkron dengan tombol yang dipilih
          ...
          <div className="mb-8">
            ...
            >
              <div className="relative z-10 text-left">
                ...
                <p className="text-xs text-gray-300 mt-1">{isScanning ? 'Mohon tunggu sebentar ya.' : 'Biarkan AI yang mencatat untukmu.'}</p>
              </div>
            ...
            <div className="flex items-center gap-4 my-6">
              ...
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Atau Manual</span>
              <div className="h-px bg-gray-200 flex-1"></div>
            </div>
          </div>
