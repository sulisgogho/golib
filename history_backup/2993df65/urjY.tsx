        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-6 pb-10">
              ...
              {hasPartner ? (
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`py-3 rounded-xl font-bold text-sm transition-all ${paidBy === userName ? 'bg-[#0B4D3C] text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}
                  >
                    {userName}
                  </button>
                  <button
                ...
                <div className="bg-gray-50 p-4 rounded-xl text-center border border-dashed border-gray-200">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Dicatat sebagai pengeluaran pribadi</p>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              {/* JIKA ADA PASANGAN, TAMPILKAN TOMBOL PILIHAN */}
              {account.partner_1_name ? (
                <div className="grid grid-cols-2 gap-3">
                  <button
                    type="button"
                    onClick={() => setPaidBy(account.client_name.split(' ')[0])}
                    className={`py-3 rounded-xl font-bold text-sm transition-all ${paidBy === account.client_name.split(' ')[0] ? 'bg-[#0B4D3C] text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}
                  >
                    {account.client_name.split(' ')[0]}
                  </button>
                  <button
                    type="button"
                    onClick={() => setPaidBy(account.partner_1_name)}
                    className={`py-3 rounded-xl font-bold text-sm transition-all ${paidBy === account.partner_1_name ? 'bg-[#0B4D3C] text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}
                  >
                    {account.partner_1_name}
                  </button>
                </div>
              ) : (
                /* JIKA SINGLE, TAMPILKAN INFO OTOMATIS (ATAU SEMBUNYIKAN) */
                <div className="bg-gray-100 p-4 rounded-xl text-center">
                  <p className="text-xs font-bold text-gray-500 uppercase">Otomatis dicatat sebagai pengeluaran Anda</p>
                  {/* Input hidden agar data tetap terkirim ke database */}
                  <input type="hidden" name="paid_by" value={account.client_name.split(' ')[0]} />
                </div>
              )}
            </div>
          </div>
