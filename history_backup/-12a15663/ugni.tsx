...
export default function EditClientPage() {
  ...
  const handleDelete = async () => {
    // Tambahkan pengecekan 'if (!account) return' agar tidak error saat data belum ada
    if (!account) return
  ...
  if (!account) {
    return (
      <div className="w-full h-screen flex items-center justify-center bg-[#F7F9F8]">
        <Loader2 className="w-8 h-8 animate-spin text-[#0B4D3C]" />
        <p className="ml-2 font-bold text-[#0B4D3C]">Memuat Data...</p>
      </div>
    )
  }
